...
from PCA_CNN import sanity_svm
...
def load(fn):
    ...
    original = torch.load(dirs[fn])  # for debugging
    data = PCA.my_pca(original)
    targets = torch.load(dirs[str(fn + '_label')])
    result = [(np.asarray(data[i]), int(targets[i]), original[i]) for i in range(1000)]
    return result
...
def run_me():
    ...
    train_o = [item[2] for item in train]
    test_data = [item[0] for item in test]
    test_l = [item[1] for item in test]
    test_o = [item[2] for item in test]
    return ((train_data, test_data), (train_l, test_l), (train_o, test_o))
